          ...
          <div className="flex-1 min-w-0">
            <div className="flex gap-4 mb-8">
              ...
              <div className="flex flex-col items-center w-12 pt-1 gap-2">
                <button onClick={() => handleVote('up')} className="text-[#bbc0c4] hover:text-orange-500 transition-colors text-4xl">
                  <HiChevronUp />
                </button>
                <span className="text-xl font-medium text-[#6a737c]">{thread.vote_score || 0}</span>
                <button onClick={() => handleVote('down')} className="text-[#bbc0c4] hover:text-orange-500 transition-colors text-4xl">
                  <HiChevronDown />
                </button>

                <button onClick={handleLike} className="mt-2 text-[#bbc0c4] hover:text-red-500 flex flex-col items-center gap-1 transition-colors">
                ...
                <button onClick={handleBookmark} className={`mt-2 hover:text-orange-500 transition-colors text-2xl ${isBookmarked ? 'text-orange-500' : 'text-[#bbc0c4]'}`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="border-t border-[#e3e6e8] pt-6 flex justify-between items-center mb-4">
                  <h2 className="text-[19px] text-[#232629] font-normal">{comments.length} Answer{comments.length !== 1 ? 's' : ''}</h2>
                </div>
                ...
                {currentUser ? (
                  <div className="mt-8">
                    ...
                    <CommentForm onSubmit={handleCommentSubmit} isLoading={isSubmitting} placeholder="Write your answer here..." />
                  </div>
                ) : (
                  <div className="mt-8 p-4 bg-[#fdf7e3] border border-[#e8d5a3] rounded text-sm text-[#6a737c]">
                    Please <a href="/login" className="text-[#0074cc]">log in</a> to post an answer.
                  </div>
                )}
              </div>
            </div>
